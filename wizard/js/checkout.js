...
function loadMyProducts(){
	var out = '';
	 for(var key in cart){
	 	out += '<div class="resultProduct"><img src="../'+data[key].image+'">';
	 	...
	 	currency = data[key].cost;
		  	...
		  	out+='<p>'+ currency +'</p></div>';
	 }
		out += '<div class="check">';
		out += '<span><input type="checkbox" checked="checked" id="shipping" onclick="checkAddress(this)"/>';
		out += '<label for="shipping">Доставка</label>';
			var shippingNum = 350;
			if(chekCrr == 1){
				shippingNum += ' $';
			}else{
		...
		out += '<p class="shippingNum">'+ shippingNum +'</p></span>';
		out += '</div>';
...
function checkAddress(checkbox){
   if (checkbox.checked){
   	...
   	shippingNum = 0;
   		...
   		if(chekCrr == 1){
   			shippingNum *= dollarRate;
		  		shippingNum = shippingNum.toFixed(2);
				shippingNum += '$'; 
			}else{
				shippingNum += ' сом';
			}
   document.querySelector('.shippingNum').innerHTML = shippingNum;
}
